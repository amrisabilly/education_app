...
@section('content')
    <div class="max-w-3xl">
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <form action="{{ route('guru.courses.lessons.store', $course) }}" method="POST">
                @csrf
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="content" rows="12" required
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                            placeholder="Tulis konten materi di sini (mendukung HTML)...">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                        <p class="mt-1 text-xs text-gray-500">Anda bisa menggunakan HTML untuk format teks</p>
                    </div>
