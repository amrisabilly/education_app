...
@section('content')
    <div class="max-w-3xl">
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <form action="{{ route('guru.courses.assignments.update', [$course, $assignment]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="6" required
                            class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">{{ old('description', $assignment->description) }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
