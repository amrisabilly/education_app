    ...
    <section class="bg-gradient-to-r from-blue-600 to-blue-800 text-white px-4 md:px-6 lg:px-8 py-8">
        <div class="max-w-4xl mx-auto">
            <div class="flex items-center gap-2 mb-4 text-sm text-blue-100">
                ...
                <span>/</span>
                <a href="{{ route('student.module.show', ['id' => $moduleId]) }}" class="hover:text-white">Modul
                    {{ $moduleId }}</a>
                <span>/</span>
                <span>Kuis</span>
            </div>
        ...
        <div class="max-w-4xl mx-auto">
            ...
            <form action="{{ route('student.module.quiz.submit', ['id' => $moduleId]) }}" method="POST">
                ...
                @for ($i = 1; $i <= $quiz['totalQuestions']; $i++)
                    <div class="bg-white rounded-2xl shadow-sm p-8 mb-8">
                        ...
                        <div class="mb-8">
                            ...
                            @elseif($i == 2)
                                <p class="text-lg text-gray-700 leading-relaxed">Jika 2x + 5 = 13, maka nilai dari x adalah?
                                </p>
                            @elseif($i == 3)
                        ...
                        <div class="space-y-4 mb-8">
                            ...
                            @foreach ($options as $label => $text)
                                <label
                                    class="flex items-start p-6 border-2 border-gray-200 rounded-xl hover:border-blue-400 hover:bg-blue-50 transition cursor-pointer group">
                                    <!-- Using name array for form submission -->
                                    <input type="radio" name="answers[{{ $i }}]" value="{{ $label }}"
                                        class="w-5 h-5 text-blue-600 cursor-pointer mt-1 flex-shrink-0" />
                ...
                <div class="flex gap-4">
                    <a href="{{ route('student.module.show', ['id' => $moduleId]) }}"
                        class="flex-1 px-6 py-4 bg-white border-2 border-gray-300 text-gray-900 font-bold rounded-lg hover:bg-gray-50 transition text-center">
                        Batal
                    </a>
                    <button type="submit"
                        class="flex-1 px-6 py-4 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition">
                        Selesaikan Kuis
                    </button>
                </div>
            </form>
